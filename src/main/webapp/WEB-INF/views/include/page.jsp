...
<c:if test="${page.curBlock gt 1 }">
	...
	<a class='page_prev' title='이전' onclick='go_page(${page.beginPage - page.blockPage})'>이전</a>	<!-- 이전 블럭 첫 페이지 -->
	<%-- <a class='page_prev' title='이전' onclick='go_page(${page.beginPage -1})'>이전</a>	<!-- 이전 블럭 마지막 페이지 --> --%>

</c:if>
	<c:forEach var="board_code" begin="${page.beginPage}" end="${page.endPage}">
		<c:if test="${board_code eq page.curPage}"> <!-- 현재페이지인 경우 -->
			<span class='page_on'>${no}</span>
		</c:if>
		<c:if test="${board_code ne page.curPage}"> <!-- 현재페이지가 아닌 경우 -->
			<a class='page_off' onclick="go_page(${board_code})">${board_code}</a>
		</c:if>
	</c:forEach>
...
.page_next, .page_last, .page_first, .page_prev {
	width:30px; height:32px; border:1px solid gray; text-indent:-999999999999px
}
