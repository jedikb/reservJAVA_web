	...
	<tr>
		...
		<td colspan='5' class='left'>${vo.board_file}
			<c:if test="${not empty vo.board_file}">
				...
				<a href='download.no?board_code=${vo.board_code}'><i class='fas fa-download'></i></a>
			</c:if>
		</td>	
	</tr>
</table>
...
<div class='btnSet'>
	...
	<c:if test="${loginInfo.member_code eq vo.board_member_code }">
		<a class='btn-fill' href='modify.no?board_code=${vo.board_code}'>수정</a>
		<a class='btn-fill' onclick="if(confirm('정말 삭제하시겠습니까?')){location='delete.no?board_code=${vo.board_code}'}">삭제</a>
	</c:if>
</div>
...
function comment_list() {
	$.ajax({
		url: 'notice/comment/${vo.board_code}',
		success: function( response ) {
